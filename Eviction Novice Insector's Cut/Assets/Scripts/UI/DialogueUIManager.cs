...
public class DialogueUIManager : MonoBehaviour
{
    ...
    [Header("Text Speed")]
    [SerializeField] float textAnimateSpeed = 0.02f;
    [SerializeField] float commaPauseMultiplier = 10;
    [SerializeField] float sentenceEndPauseMultiplier = 20;
    float textAnimateSpeedMultiplier = 1;
    ...
    void UpdateUI(VD.NodeData data)
    {
        ...
        else  //If it's an NPC Node, let's just update NPC's text and sprite
        {
            ...
            NPC_TextAnimator = DrawText(data.comments[data.commentIndex], textAnimateSpeed * textAnimateSpeedMultiplier);
            ...
            UpdateNameMaterial(NPC_label.text);

            //Sets the NPC container on
            NPC_Container.SetActive(true);
            PunchDown(NPCPhotoFrame);
        }
    }
    ...
    IEnumerator DrawText(string text, float time)
    {
        ...
        for (int i = 0; i < words.Length; i++)
        {
            ...
            for (int j = 0; j < word.Length; j++)
            {
                ...
                else if (word.Substring(0, j + 1).Contains(". ") || word.Substring(0, j + 1).Contains("! ") || word.Substring(0, j + 1).Contains("? "))
                {
                    textAnimateSpeedMultiplier = sentenceEndPauseMultiplier;
                }
                else if (textAnimateSpeedMultiplier > 1)
                {
                    textAnimateSpeedMultiplier = 1;
                }
                yield return new WaitForSeconds(textAnimateSpeed * textAnimateSpeedMultiplier);
            }
        }
        NPC_Text.text = text;
        animatingText = false;
